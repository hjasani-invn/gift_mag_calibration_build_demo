...
def Local2GeoAllParticles( one_time_particles):
    ...
    time = float(one_time_particles[0][1])
    geopositions = []
    for one_time_particle in one_time_particles:
        #print(one_time_particle)
        t = float(one_time_particle[1])/1000
        x = float(one_time_particle[2])
        y = float(one_time_particle[3])
        z = float(one_time_particle[4])
        w = float(one_time_particle[5])
        type = float(one_time_particle[6])
        #print("x = ", x, "  y = ", y)
        lat, lon = Local2Geo( x,  y )
        geopositions.append([t, lat, lon, z, w, type])
