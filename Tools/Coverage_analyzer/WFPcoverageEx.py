...
def getWFPcoverage(gridFile, db3_file, settings_json, floor):
    ...
    # maxF = int(settings['venue']['floors_count']) - 1
...
def plot_WFP_coverage(numMeasPerAP, settings_json, output_directory, output_plot_file_name, output_lat_lon_file, floor):
    settings = json.load(open(settings_json))
    ...
    plt.title('Amount of WiFi measurements per AP')
    plt.colorbar()
