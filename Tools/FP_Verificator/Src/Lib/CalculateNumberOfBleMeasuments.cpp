...
namespace FPVerificator
{
        ...
        )
    {
        ...
        for (auto it = irlpositions.begin(); it != irlpositions.end(); it++)
        {
            ...
            double x = 0, y = 0;
            bool flag;
            double min_distance = ble_grid_size * sqrt(2.0);
            double distance;

            nearcelllist.clear();

            for (uint32_t k = 0; k < bleGrid.size(); k++)
            {
                flag = false;
                x = bleGrid[k].coordinates.x;
                y = bleGrid[k].coordinates.y;
#if 0  
...
#endif
                {
                    ...
                    //flag = true;
                    //break;
                    std::sort(bleGrid[k].bleData[0].scanBle.begin(), bleGrid[k].bleData[0].scanBle.end(), compare_RSSIMeasurement_by_mac);
                    int32_t  macaddressCounter = 0;
                    uint64_t prevMacAddress = 0;
                    for (auto scanBleIt = bleGrid[k].bleData[0].scanBle.begin(); scanBleIt != bleGrid[k].bleData[0].scanBle.end(); scanBleIt++)
                    {
                        uint64_t currentMacAddress = scanBleIt->mac;
                    ...
                    nearcell.number_of_measurements = bleGrid[k].bleData[0].scanBle.size() / macaddressCounter;
                    nearcell.X = x;
                    nearcell.Y = y;
            ...
            {
                x = ((uint32_t)(pos.X)) + ble_grid_size / 2;
                y = ((uint32_t)(pos.Y)) + ble_grid_size / 2;
            }
