...
BSSID  Ble_ApInfo::string_to_mac( std::string const &s )
{
    unsigned int a[7];
    int last = -1;
    unsigned int band = 0;
...
//}
 ...
 Ble_Location operator* ( const  Ble_Location &L, const tProb &w )
{
    Ble_Location loc = L;
    loc.x *= w;
    loc.y *= w;
 ...
 Ble_Location operator/( const  Ble_Location &L, const tProb &w )
{
 ...
 Ble_Location &operator/=(  Ble_Location &L, const tProb &w )
{
    L.x /= w;
    L.y /= w;
    L.z /= w;
 ...
 Ble_Location operator+( const  Ble_Location &L, const  Ble_Location &R )
{
    Ble_Location loc = L;
    loc.x += R.x;
    loc.y += R.y;
 ...
 Ble_Location &operator+=(  Ble_Location &L, const  Ble_Location &R )
{
    L.x += R.x;
    L.y += R.y;
    L.z += R.z;
...
void  Ble_Location::set( const Coordinates _x, const Coordinates _y, const Coordinates _z,  const tProb _p , const bool _valid )
{
    x = _x;
    y = _y;
    z = _z;
...
Coordinates  Ble_Location::norm(const  Ble_Location &x0) const
{
