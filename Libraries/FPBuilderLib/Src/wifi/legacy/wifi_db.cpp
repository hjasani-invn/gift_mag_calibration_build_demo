...
WiFi_DB::WiFi_DB( const Fpbl::WiFiGrid &wifiGrid )
{
    for ( auto pos_it = wifiGrid.cbegin(); pos_it != wifiGrid.cend(); ++pos_it )
    {
        DBRecord rec;
        Fpbl::CoordinatesInGrid pos = pos_it->coordinates;
        rec.location.set( pos.x, pos.y, pos.floor, true, 1. );

        for ( auto scan_it = pos_it->wifiData.cbegin(); scan_it != pos_it->wifiData.cend(); ++scan_it )
        {
            if ( scan_it->scanWiFi.size() == 0 ) continue;

            APMeas rawScan;

            for ( auto ap_it = scan_it->scanWiFi.begin(); ap_it != scan_it->scanWiFi.end(); ++ap_it )
            {
...
tProb WiFi_DB::hist2prob( const WiFi_DB::APData &fp, int rssi, tProb kernel_sig )
{
    tProb p_rssi = 0;
