...
void
polygon_get_bbox(const polygon_t *polygon, float *bbox)
{
    edgelist_t *edgelist;
    bbox[0] = 100000.0;
    bbox[1] = 100000.0;
    ...
    list_for_each_entry(edgelist, edgelist_t, &polygon->edge_list, list) {
        bbox[0] = MIN(edgelist->edge.start.X, bbox[0]);
        bbox[1] = MIN(edgelist->edge.start.Y, bbox[1]);
        bbox[2] = MAX(edgelist->edge.start.X, bbox[2]);
        bbox[3] = MAX(edgelist->edge.start.Y, bbox[3]);
    }
}
...
// #ifndef LINUX_WORKSTATION_OS
// void
// plot_polygon(const polygon_t *polygon)
// {
//     edgelist_t *edgelist;
//     log_info(debug_shape, "%d\n", polygon->num);
//     list_for_each_entry(edgelist, edgelist_t, &polygon->edge_list, list) {
//         log_info(debug_shape, "%f %f\n", edgelist->edge.start.X, edgelist->edge.start.Y);
//     }
// }
// #endif
